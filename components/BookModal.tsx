  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-xl flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-[#181818] border border-white/10 rounded-2xl shadow-2xl p-8 w-full max-w-lg flex flex-col animate-fade-in-up" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-white mb-6">{bookToEdit ? 'Editar Livro' : 'Adicionar Livro'}</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-gray-300 mb-1 block">Autor*</label>
          ...
          <div>
            <label className="text-sm font-medium text-gray-300 mb-1 block">Vincular a Recurso</label>
        ...
        <div className="w-full flex gap-4 mt-8 pt-6 border-t border-white/10">
            ...
            <button onClick={handleSubmit} className="w-full bg-[#00A9FF] text-black font-bold py-3 rounded-lg hover:bg-opacity-90 shadow-lg shadow-[#00A9FF]/20">Salvar Livro</button>
        </div>
      </div>
    </div>
  );
