  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-xl flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-[#181818] border border-white/10 rounded-2xl shadow-2xl p-8 w-full max-w-3xl flex flex-col animate-fade-in-up max-h-[95vh]" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-white mb-6">{goalToEdit ? 'Editar Meta' : 'Nova Meta'}</h2>
        ...
        <div className="flex-grow overflow-y-auto pr-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                 ...
                 <div>
                    <label className="text-sm font-medium text-gray-300 mb-1 block">Área de Desenvolvimento*</label>
                 ...
                 <div>
                    <label className="text-sm font-medium text-gray-300 mb-1 block">Horizonte*</label>
                    <select name="horizon" value={goal.horizon} onChange={handleChange} className="w-full bg-gray-800/60 p-2 rounded-lg border border-gray-700">
                        ...
                        <option>Médio Prazo (3-12 meses)</option>
                        <option>Longo Prazo (>1 ano)</option>
                    </select>
                </div>
                <div>
                    <label className="text-sm font-medium text-gray-300 mb-1 block">Data Limite*</label>
            ...
            <div>
             ...
             <div>
                <label className="text-sm font-medium text-gray-300 mb-1 block">Resultados Esperados</label>
             ...
             <div>
                <label className="text-sm font-medium text-gray-300 mb-1 block">Métricas de Sucesso</label>
            ...
            <div>
                 <label className="text-sm font-medium text-gray-300 mb-1 block">Tags (separadas por vírgula)</label>
            ...
            <div className="border-t border-white/10 pt-4">
                <h3 className="text-lg font-semibold text-white mb-2">Marcos Milestones</h3>
                <div className="space-y-2 mb-3 max-h-40 overflow-y-auto">
                    {goal.milestones.map(m => (
                        <div key={m.id} className="flex items-center justify-between bg-gray-800/50 p-2 rounded">
                            <span className="text-sm">{m.name} - {new Date(m.date + 'T12:00:00').toLocaleDateString('pt-BR')}</span>
                ...
                <div className="flex gap-2 items-center">
                    <input value={newMilestoneName} onChange={e => setNewMilestoneName(e.target.value)} placeholder="Nome do marco" className="flex-grow bg-gray-700/60 p-2 rounded-lg border border-gray-600 text-sm"/>
                    ...
                    <button onClick={handleAddMilestone} className="bg-white/10 text-white font-semibold px-3 py-2 rounded-lg text-sm hover:bg-white/20">+</button>
                </div>
            </div>
        </div>
        
        <div className="w-full flex gap-4 mt-6 pt-6 border-t border-white/10">
            <button onClick={onClose} className="w-full bg-gray-700/80 text-white font-bold py-3 rounded-lg hover:bg-gray-700">Cancelar</button>
            <button onClick={handleSubmit} className="w-full bg-[#00A9FF] text-black font-bold py-3 rounded-lg hover:bg-opacity-90 shadow-lg shadow-[#00A9FF]/20">Salvar Meta</button>
        </div>
      </div>
    </div>
  );
