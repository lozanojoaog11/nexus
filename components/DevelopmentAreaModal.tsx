  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-xl flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-[#181818] border border-white/10 rounded-2xl shadow-2xl p-8 w-full max-w-2xl flex flex-col animate-fade-in-up max-h-[95vh]" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-white mb-6">{nodeToEdit ? 'Editar Área' : 'Nova Área de Desenvolvimento'}</h2>
        
        <div className="flex-grow overflow-y-auto pr-4 space-y-4">
            <div>
                <label className="text-sm font-medium text-gray-300 mb-1 block">Nome da Área*</label>
            ...
            <div>
                <label className="text-sm font-medium text-gray-300 mb-1 block">Tipo de Área</label>
            ...
            <div>
                ...
                <textarea name="successMetrics" value={node.successMetrics} onChange={handleChange} rows={2} placeholder="Como você medirá o progresso nesta área?" className="w-full bg-gray-800/60 p-2 rounded-lg border border-gray-700"/>
            </div>
             <div>
                <label className="text-sm font-medium text-gray-300 mb-1 block">Data Alvo</label>
        ...
        <div className="w-full flex gap-4 mt-6 pt-6 border-t border-white/10">
            ...
            <button onClick={handleSubmit} className="w-full bg-[#00A9FF] text-black font-bold py-3 rounded-lg hover:bg-opacity-90 shadow-lg shadow-[#00A9FF]/20">Salvar Área</button>
        </div>
      </div>
    </div>
  );
