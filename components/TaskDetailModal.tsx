    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-2xl flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-[#181818] border border-white/10 rounded-2xl shadow-2xl p-8 w-full max-w-2xl flex flex-col space-y-6 animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label htmlFor="task-status" className="text-sm font-medium text-gray-300 mb-2 block">{t('taskModal.status')}</label>
                <select
                    id="task-status"
                    value={status}
                ...
                >
                    <option value="A Fazer">{t('kanban.todo')}</option>
                    <option value="Em Progresso">{t('kanban.inProgress')}</option>
                    <option value="Concluído">{t('kanban.done')}</option>
                </select>
            </div>
             <div>
                <label htmlFor="task-deadline" className="text-sm font-medium text-gray-300 mb-2 block">{t('taskModal.deadline')}</label>
                <input
                    id="task-deadline"
                    type="date"
                    value={deadline}
             ...
             <div className="relative md:col-span-2">
                <label htmlFor="task-development" className="text-sm font-medium text-gray-300 mb-2 block">{t('taskModal.linkToDev')}</label>
                ...
                >
                    <option value="">{t('taskModal.noLink')}</option>
                    {developmentNodes.map(node => (
                        <option key={node.id} value={node.id}>[{t(`devAreaModal.types.${node.type.replace(/\s/g, '').replace(/[()]/g, '')}`, {defaultValue: node.type})}] {node.label}</option>
                    ))}
                </select>
            </div>
            <div className="md:col-span-2">
                <label htmlFor="task-notes" className="text-sm font-medium text-gray-300 mb-2 block">{t('taskModal.notes')}</label>
                <textarea
                    id="task-notes"
                    value={notes}
