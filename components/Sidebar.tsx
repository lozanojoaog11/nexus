...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, onSignOut }) => {
  ...
  return (
    <div className="w-64 bg-black/20 backdrop-blur-xl border-r border-white/10 p-6 flex flex-col flex-shrink-0 h-full">
      <div className="flex items-center space-x-3 mb-8">
        <EixoLogo className="w-8 h-8 text-gray-400"/>
        <h1 className="text-2xl font-bold text-white tracking-widest">EIXO</h1>
      </div>
      <Clock />
      <nav className="flex flex-col space-y-3 mt-8 flex-grow overflow-y-auto pr-2">
        {NAV_ITEMS.map(item => (
          <button
            key={item.view}
